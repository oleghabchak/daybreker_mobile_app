...
interface FileInfo {
  ...
  imports: string[];
  exports: string[];
  usesOldTheme: boolean;
  usesNewComponents: boolean;
  complexity: number;
}
...
class DependencyAnalyzer {
  ...
  private analyzeFile(filePath: string, content: string): FileInfo {
  ...
  private calculateMigrationOrder(): string[] {
    ...
    const sorted = Array.from(this.files.entries()).sort(([pathA, infoA], [pathB, infoB]) => {
      // Constants first
      if (pathA.includes('/constants/')) return -1;
      if (pathB.includes('/constants/')) return 1;
      
      // UI components second
      if (pathA.includes('/components/ui/')) return -1;
      if (pathB.includes('/components/ui/')) return 1;
      
      // Other components third
      if (pathA.includes('/components/')) return -1;
      if (pathB.includes('/components/')) return 1;
      
      // Screens last
      if (pathA.includes('/screens/')) return 1;
      if (pathB.includes('/screens/')) return -1;
  ...
  private generateReport(migrationOrder: string[]) {
    const report = {
      summary: {
