-- Migration: Fix calibration trigger for aggressive recalculation
-- Issue: Trigger wouldn't re-fire if user toggled checkbox, and wouldn't overwrite Set 2 if it already had values
-- Fix: Remove OLD.status check AND remove empty-check to always recalculate Set 2 from latest calibration data
-- 
-- USE CASE: User checks calibration → Set 2 populated → User changes reps → User re-checks → Set 2 recalculated
...
DECLARE
    ...
    next_week INTEGER;
BEGIN
    -- ✅ CHANGE 1: Remove OLD.status check to allow re-triggering
    -- OLD: IF NEW.status != 'done' OR OLD.status = 'done' THEN
    -- NEW: Only check if NEW.status is 'done'
    ...
    IF current_set_type = 'calibration' AND current_set_number = 1 THEN
        ...
        -- ✅ CHANGE 2: Remove empty-check to ALWAYS recalculate Set 2
        -- OLD: IF next_set_record.id IS NOT NULL AND (weight_kg IS NULL OR weight_kg = 0) AND (actual_reps IS NULL OR actual_reps = 0) THEN
        -- NEW: Just check if Set 2 exists, always update it
        IF next_set_record.id IS NOT NULL THEN
            ...
            BEGIN
                ...
                RAISE NOTICE '✅ RECALCULATED validation set from calibration: original_weight=%, calibration_reps=%, working_weight=%, validation_reps=%', 
